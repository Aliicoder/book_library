...
export const GET = CatchAsyncError(async (req: NextRequest) => {
  ...
  conditions.push(eq(borrowRecords.status, 'PENDING'))

  const borrowRequestsList = await db
    .select({
    ...
    .limit(limit)
    .offset(offset)
  console.log('borrowRequestsList ', borrowRequestsList)
  ...
  return Response.json({
    borrowRequests: borrowRequestsList,
    totalRequests: total,
